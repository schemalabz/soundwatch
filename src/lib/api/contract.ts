...
import type { READING_COLUMNS, ReadingRow } from "./readings";
import type { ReadingRow as IngestRow } from "../../../mqtt-ingester/row";
import type { COLUMNS } from "../../../simulator/backfill";
 ...
 * lib/api/readings) and COLUMNS (write path, in the simulator backfill). Where
...
type _ColumnMapsAgree = Expect<
  NoneLeft<
    ColumnDisagreements,
    "The read map (READING_COLUMNS in lib/api/readings) and the write map (COLUMNS in simulator/backfill) give this field two different column names. Make them match."
  >
>;
